...
typedef struct {
    ...
    int         *arr;              // the full integer array to sort
    int          left, right;      // this child’s subarray bounds
    uv_mutex_t  *mutex;            // pointer to a shared parent mutex
    int         *parent_counter;   // pointer to a shared parent counter
    int          parent_left;      // parent’s left bound (for merge)
    int          parent_right;     // parent’s right bound (for merge)
} sort_baton_t;
...
static void after_cb(uv_work_t *req, int status) {
    sort_baton_t *baton = req->data;
    fprintf(stderr,
            "[after_cb] thread %lu finished [%d..%d]\n",
            (unsigned long)uv_thread_self(),
            baton->left,
            baton->right);
